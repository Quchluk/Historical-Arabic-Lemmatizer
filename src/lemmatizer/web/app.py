...
# Import from lemmatizer package
from lemmatizer.utils.embed_texts import TextExtractor, AraBERTEmbedder
from lemmatizer.config import LOGS_DIR, STOP_WORDS_FILE, NON_ARABIC_WORDS_FILE
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOGS_DIR / 'word_context_analysis.log'),
...
class OnTheFlyAnalyzer:
    ...
    def __init__(self, metadata_file: str, output_dir: str = 'word_context_analysis'):
        ...
        self.arabic_stop_words = self._load_stop_words(STOP_WORDS_FILE)
        self.non_arabic_words = self._load_non_arabic_words(NON_ARABIC_WORDS_FILE)
    ...
    def process_all_texts(self, texts_per_batch=10):
        ...
        for idx, row in tqdm(valid_texts.iterrows(), total=len(valid_texts), desc="Processing"):
            # Resolve full path using DB_DIR from config
            # Metadata path is like "db/0025AH/..." or "0025AH/..."
            rel_path = row['text_file_path']
            if rel_path.startswith('db/'):
                rel_path = rel_path[3:]

            # The structure is data/db/db/0025AH...
            # DB_DIR points to data/db
            # So we need DB_DIR / "db" / rel_path
            full_path = DB_DIR / "db" / rel_path

            success = self.process_single_text(str(full_path), row['version_uri'])
...
from lemmatizer.config import RAW_DATA_DIR, DATA_DIR, DB_DIR
...
def main():
    ...
    analyzer = OnTheFlyAnalyzer(
        metadata_file=str(RAW_DATA_DIR / "openiti_metadata.parquet"),
        output_dir=str(DATA_DIR / "jobs/TF_stat/word_context_analysis")
    )
