...
import pickle
from lemmatizer.config import LOGS_DIR, RAW_DATA_DIR, DATA_DIR
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOGS_DIR / 'text_embedding.log'),
...
class TextEmbeddingGenerator:
    ...
    def __init__(self, db_root: str = None, metadata_file: str = None,
                 output_dir: str = None, batch_size: int = 1):
        """
        ...
        """
        self.db_root = Path(db_root) if db_root else DATA_DIR / "db/db"
        self.metadata_file = Path(metadata_file) if metadata_file else RAW_DATA_DIR / "openiti_metadata.parquet"
        self.output_dir = Path(output_dir) if output_dir else DATA_DIR / "embeddings"
...
def main():
    ...
    parser.add_argument('--db-root', default=None, help='Root directory of OpenITI database')
    parser.add_argument('--metadata', default=None, help='Metadata parquet file')
    parser.add_argument('--output-dir', default=None, help='Output directory for embeddings')
